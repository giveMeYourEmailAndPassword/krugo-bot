...
func (b *BridgeClient) Analyze(ctx context.Context, rawText string) (string, error) {
	...
	prompt := fmt.Sprintf(
		"Новая заявка от менеджера. Проанализируй и дай структурированный ответ:\n\n%s\n\n"+
			"Ответь в формате: ID, Статус, Клиент, Проект, Тип, Актуальность, Риск, Нужны уточнения, Рекомендация.",
		rawText,
	)

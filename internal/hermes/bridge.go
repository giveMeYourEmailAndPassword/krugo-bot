...
func (b *BridgeClient) Analyze(ctx context.Context, rawText string) (string, error) {
	...
	prompt := fmt.Sprintf(
			"⚠️ Сейчас режим чтения: сделай GET договора, покажи текущие данные и что БУДЕТ изменено.\n"+
			"PATCH выполняй только если в заявке явно написано «ПОДТВЕРЖДАЮ ВЫПОЛНЕНИЕ».\n"+

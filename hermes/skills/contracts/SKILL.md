...
### ПЕРВЫЙ ШАГ — ПРЕФЛАЙТ (обязательно!)

**ПЕРЕД любой работой выполни одну команду:**
```bash
ls /opt/data/skills/contracts/tools/create_payment.sh /opt/data/skills/contracts/tools/pb_helper.sh
```
**Если результат пустой или "No such file" → СТОП. Не делай НИЧЕГО дальше.
Не создавай пользователей, не делай curl, не пытайся обойти. Просто ответ:**
«ОШИБКА: инструменты не смонтированы (tools/ пуст). Нужен redeploy контейнера.
...
**Запрещено:**
- Создавать пользователей в PB (тестовых или любых других)
- Прямой POST/PATCH/DELETE к PB (минуя скрипты)
- GET к PB для проверки — разрешён, но если скрипты отсутствуют — СТОП целиком

**Единственные разрешённые curl-запросы — GET (чтение) для проверки статусов,
и ТОЛЬКО если префлайт прошёл (скрипты существуют).**
